...
### 1. 複数ページ管理（タブUI）
- **タブ操作**:
  - **＋ボタンの追従配置**: Windows 11 純正メモ帳と同様、最終タブのすぐ右隣に `＋` ボタンが追従配置され、直感的に新規ページを追加（`Ctrl+T` にも対応）。
  - **インラインでのタイトル変更**: タブ名をダブルクリック、または `F2` キーを押すとタブ上がテキストボックスになり、その場で直接名前を変更可能（Enterで確定、Escでキャンセル）。メニューや右クリックから「1行目をページ名に設定」も可能。
  - **タブのドラッグ＆ドロップ並べ替え**: マウスでタブを掴んでドラッグするだけで、自由な位置にタブを並べ替え可能。
  - `✕` ボタンまたは `Ctrl+W` でページを閉じる。
  - `Ctrl+Tab` / `Ctrl+Shift+Tab` でキーボードだけで前後のタブを素早く巡回・切り替え。
  - 右クリックコンテキストメニュー（名前の変更、1行目をページ名に設定、ページの複製、左右並べ替え、テキスト出力、ページを閉じる）。
