...
- **タブ操作**:
  ...
  - `Ctrl+Tab` / `Ctrl+Shift+Tab` でキーボードだけで前後のタブを素早く巡回・切り替え。
  - タブ名をダブルクリック、または `F2` キーで直接名前変更。
  - 右クリックコンテキストメニュー（名前の変更、ページの複製、左右並べ替え、テキスト出力）。
  - 未保存の変更があるタブには変更インジケータ（●）を表示。
  - **エディタ状態の完全復元**: タブを何度切り替えても、各ページごとのカーソル位置やスクロール位置を自動記憶・復元。切り替えた瞬間にそのまま続きを入力できます。

### 2. ファイルのドラッグ＆ドロップ (Drag & Drop) 対応
- エクスプローラーやデスクトップからファイルを NoteX ウィンドウへ直接ドラッグ＆ドロップ可能。
  - **`.txtx` ファイル**: 保存確認の上でドキュメントとして即座に開きます。
  - **テキストファイル (`.txt`, `.log`, `.csv`, `.md`, `.json`, `.cs` 等)**: 複数ファイルをまとめてドロップすると、すべて一括で新しいタブ（ページ）として取り込みます。
...
### 4. メモ帳と同等の基本編集 & Windows 11 Fluent スタイル
- **編集機能**: 高速タイピング、完全な Undo / Redo（「すべて置換」後も `Ctrl+Z` で一発復帰可能）、日付と時刻の挿入 (`F5`)。
- **検索・置換**: `Ctrl+F`（検索）、`Ctrl+H`（置換）、`F3` / `Shift+F3`（次/前を検索）。
- **表示設定**:
  - **折り返し対応の行番号表示**: 右端で折り返している場合でも、エディタのテキスト行と行番号マージンが1ピクセルも狂わずに完全同期。
  - 右端での折り返し（ワードラップ）切替。
  - フォント名・フォントサイズの変更（設定ダイアログからリアルタイム反映）。
  - 拡大 / 縮小（`Ctrl + マウスホイール`、`Ctrl + +/-/0`）。
- **テーマ切替**:
  - システム設定に従う / ライトモード / ダークモード。
- **ステータスバー**:
  - キャレット位置（テキスト本来の論理行・論理列）、文字数（テキスト選択時は選択文字数を表示）、ズーム倍率、改行コード（CRLF / LF / CR 動的判定）、エンコーディング、ページ数インジケータ。
...
| **Ctrl + T** | 新規ページ（タブ）を追加 |
| **Ctrl + W** | 現在のページを閉じる |
| **Ctrl + Tab** | 次のページ（タブ）へ移動 |
| **Ctrl + Shift + Tab** | 前のページ（タブ）へ移動 |
| **F2 / ダブルクリック** | ページ名の変更 |
| **Ctrl + F** | インライン検索バーを表示 |
| **Ctrl + H** | インライン置換バーを表示 |
| **F3 / Shift + F3** | 次を検索 / 前を検索 |
| **Ctrl + Z** | 元に戻す（一括置換後も可能） |
| **Ctrl + Y** | やり直し |
| **F5** | 現在の日付と時刻を挿入 |
| **Ctrl + ホイール** | 画面の拡大 / 縮小 |
...
### Q. 既存のメモ帳ファイル（.txt）をドラッグ＆ドロップで追加できますか？
はい。エクスプローラーから1つまたは複数のテキストファイル（`.txt`, `.md`, `.csv`, `.log` 等）を NoteX のウィンドウ上にドラッグ＆ドロップするだけで、各ファイルを個別のタブ（ページ）として一括で取り込めます。また、`.txtx` ファイルをドロップすればそのドキュメントを開くことができます。
